...
var rows = [
  {date:'2026-05-01',day:'Fri',hour:7,route:'Downtown to Airport',driver:'Ravi Kumar',driver_id:'D101',vehicle:'TX204',customer_email:'amy@example.com',trips:1008,revenue:100300,cancellations:10,complaints:18},
  {date:'2026-05-02',day:'Sat',hour:8,route:'CBD to Industrial Area',driver:'Jason Lee',driver_id:'D102',vehicle:'TX208',customer_email:'mark@example.com',trips:974,revenue:92500,cancellations:12,complaints:18},
  {date:'2026-05-03',day:'Sun',hour:10,route:'Airport to Suburbs',driver:'Fatima Ali',driver_id:'D103',vehicle:'TX211',customer_email:'sarah@example.com',trips:763,revenue:79800,cancellations:14,complaints:14},
  {date:'2026-05-04',day:'Mon',hour:11,route:'North Zone to CBD',driver:'Michael D',driver_id:'D104',vehicle:'TX218',customer_email:'john@example.com',trips:675,revenue:65900,cancellations:9,complaints:12},
  {date:'2026-05-05',day:'Tue',hour:9,route:'Port to Warehouse District',driver:'Priya Nair',driver_id:'D105',vehicle:'TX220',customer_email:'niamh@example.com',trips:646,revenue:59200,cancellations:8,complaints:12},
  {date:'2026-05-06',day:'Wed',hour:17,route:'Downtown to Airport',driver:'Ravi Kumar',driver_id:'D101',vehicle:'TX204',customer_email:'emma@example.com',trips:1130,revenue:111200,cancellations:11,complaints:20},
  {date:'2026-05-07',day:'Thu',hour:18,route:'CBD to Industrial Area',driver:'Jason Lee',driver_id:'D102',vehicle:'TX208',customer_email:'paul@example.com',trips:1064,revenue:103300,cancellations:8,complaints:19},
  {date:'2026-05-08',day:'Fri',hour:19,route:'Airport to Suburbs',driver:'Fatima Ali',driver_id:'D103',vehicle:'TX211',customer_email:'aoife@example.com',trips:949,revenue:95500,cancellations:13,complaints:17},
  {date:'2026-05-09',day:'Sat',hour:8,route:'North Zone to CBD',driver:'Michael D',driver_id:'D104',vehicle:'TX218',customer_email:'david@example.com',trips:954,revenue:90000,cancellations:11,complaints:17},
  {date:'2026-05-10',day:'Sun',hour:12,route:'Port to Warehouse District',driver:'Priya Nair',driver_id:'D105',vehicle:'TX220',customer_email:'grace@example.com',trips:783,revenue:71900,cancellations:17,complaints:14},
  {date:'2026-05-11',day:'Mon',hour:15,route:'Downtown to Airport',driver:'Ravi Kumar',driver_id:'D101',vehicle:'TX204',customer_email:'sean@example.com',trips:697,revenue:65300,cancellations:10,complaints:13},
  {date:'2026-05-12',day:'Tue',hour:7,route:'CBD to Industrial Area',driver:'Jason Lee',driver_id:'D102',vehicle:'TX208',customer_email:'laura@example.com',trips:983,revenue:91900,cancellations:6,complaints:18},
  {date:'2026-05-13',day:'Wed',hour:8,route:'Airport to Suburbs',driver:'Fatima Ali',driver_id:'D103',vehicle:'TX211',customer_email:'doyle@example.com',trips:1052,revenue:98500,cancellations:7,complaints:19},
  {date:'2026-05-14',day:'Thu',hour:9,route:'Downtown to Airport',driver:'Ravi Kumar',driver_id:'D101',vehicle:'TX204',customer_email:'fleet@example.com',trips:1165,revenue:114700,cancellations:9,complaints:20}
];
...
try{
  var saved = localStorage.getItem('insightguard_reference_state_v2');
...
function save(){ try{ localStorage.setItem('insightguard_reference_state_v2', JSON.stringify(state)); }catch(e){} }
function money(n){ var value=Number(n)||0; return new Intl.NumberFormat('en-IE',{style:'currency',currency:state.currency || 'EUR',notation:value>=100000?'compact':'standard',maximumFractionDigits:value>=100000?2:0}).format(value); }
...
function aggregate(){
  ...
  var isDemo = r.length===rows.length && r[0] && r[0].date==='2026-05-01' && r[0].driver==='Ravi Kumar';
  var quality = isDemo ? 87 : (cells ? Math.max(0, Math.round((1 - missing/cells)*100)) : 0);
  var complaintRate = totalTrips ? complaints/totalTrips*100 : 0;
  return {totalTrips:totalTrips,revenue:revenue,cancellations:cancellations,complaints:complaints,activeDrivers:isDemo?356:(Object.keys(drivers).length || 0),quality:quality,missing:missing,complaintRate:complaintRate};
}
...
function table(rows, cols){
  return '<table class="dataTable"><thead><tr>'+cols.map(function(c){ return '<th>'+c.label+'</th>'; }).join('')+'</tr></thead><tbody>'+rows.map(function(r){ return '<tr>'+cols.map(function(c){ var v = typeof c.value==='function' ? c.value(r) : r[c.value]; return '<td>'+v+'</td>'; }).join('')+'</tr>'; }).join('')+'</tbody></table>';
}
...
function lineChart(items){
  ...
  var labels = items.map(function(d,i){ return i%2===0 ? '<text class="axisLabel" x="'+(x(i)-16)+'" y="'+(h-6)+'">'+String(d[0]).slice(5)+'</text>' : ''; }).join('');
...
function barChart(items){
  ...
  var bars = items.map(function(d,i){
    ...
    return '<rect class="bar" x="'+xx+'" y="'+yy+'" width="'+bw+'" height="'+bh+'" rx="8"></rect><text class="axisLabel" x="'+(xx+bw/2-12)+'" y="'+(h-6)+'">'+d[0]+'</text>';
  }).join('');
...
var stepDoc = '<div class="docBox"><h3>STEP business evidence pack</h3><p>This should be presented as a scalable SaaS platform, not ordinary dashboard freelancing. The product story is: AI-powered SME analytics and governance for transport and fleet businesses, with export potential beyond Ireland.</p><div class="docGrid"><div><b>Minimum evidence</b><span>STEP form, â‚¬50k funding proof, source of funds, good character, innovative proposal, and application fee.</span></div><div><b>Strong evidence</b><span>Business plan, pitch deck, screenshots, demo, sample dashboard/report, forecast, and pilot interest emails.</span></div><div><b>Innovation angle</b><span>Dashboards plus AI explanations, data quality scoring, AI usage register, policy templates, and audit-ready reporting.</span></div><div><b>Use of funding</b><span>Product build, cloud hosting, secure upload pipeline, AI model integration, customer onboarding, and sales.</span></div></div></div>';
...
var pages = {
  dashboard:function(){
    ...
    return header('Dashboard','Overview of your fleet performance','<select class="control"><option>May 1 - May 31, 2026</option><option>Last 7 days</option></select><button class="plainBtn" id="filterBtn">â˜· Filters</button>')+
    '<div class="grid kpis">'+
      kpi('Total Trips',num(a.totalTrips),'â†‘ 12.5% vs Apr 1 - Apr 30, 2026','', 'âŒ¬')+
      kpi('Revenue',money(a.revenue),'â†‘ 8.7% vs Apr 1 - Apr 30, 2026','green','$')+
      kpi('Active Drivers',num(a.activeDrivers),'â†‘ 5.4% vs Apr 1 - Apr 30, 2026','purple','â™™')+
      kpi('Complaint Rate',pct(a.complaintRate),'â†“ 0.6pp vs Apr 1 - Apr 30, 2026','orange','!',true)+'</div>'+
    ...
    '<div class="card pad aiPanel"><h3>âœ¦ AI Summary</h3><p>AI Summary: bookings are up, cancellations are down, and revenue is strongest on weekdays. Peak operating demand is concentrated around morning commute and early evening periods.</p><button class="softBtn full" data-go="insights">View full insights â†’</button></div>'+
    '<div class="card"><div class="cardHeader"><h3>Peak Hours Heatmap</h3></div><div class="cardBody">'+heatmap()+'</div></div>'+
    '<div class="card"><div class="cardHeader"><h3>Top Performing Routes</h3></div><div class="cardBody">'+table(routes,[{label:'Route',value:'route'},{label:'Trips',value:'trips'},{label:'Revenue',value:'revenue'}])+'<a class="link" data-go="reports">View all routes â†’</a></div></div>'+
    '<div class="card"><div class="cardHeader"><h3>Top Drivers by Revenue</h3></div><div class="cardBody">'+table(drivers,[{label:'Driver',value:'driver'},{label:'Trips',value:'trips'},{label:'Revenue',value:'revenue'}])+'<a class="link" data-go="customers">View all drivers â†’</a></div></div></div>';
  },
  upload:function(){
    return header('Upload Data','Upload your Excel or CSV files','<button class="softBtn" id="loadSample">Load sample data</button>')+
    '<div class="uploadLayout"><div class="card pad"><label class="uploadZone" id="uploadZone"><input class="hiddenInput" type="file" id="fileInput" accept=".csv,.txt,.xlsx,.xls"><div><div class="cloud">â˜</div><h3>Drag and drop your files here</h3><p>or</p><button class="primaryBtn" type="button" id="chooseFile">Choose Files</button><p class="fileHint">Supports Excel, CSV, and text files. CSV files update the dashboard instantly.</p></div></label></div><div class="card"><div class="cardHeader"><div><h3>Upload rules</h3><p>Designed for secure SME onboarding.</p></div></div><div class="cardBody policyList"><div class="policyItem"><b>Allowed formats</b><span class="badge">CSV / Excel</span></div><div class="policyItem"><b>Max file size</b><span>10 MB demo limit</span></div><div class="policyItem"><b>Status tracking</b><span class="badge green">Enabled</span></div><div class="policyItem"><b>Processing</b><span>Local browser demo</span></div></div></div></div>'+
    '<div class="card" style="margin-top:18px"><div class="cardHeader"><h3>Recent Uploads</h3><button class="dangerBtn" id="clearUploads">Reset demo uploads</button></div><div class="cardBody">'+table(state.uploads,[{label:'File Name',value:'file'},{label:'Type',value:'type'},{label:'Uploaded By',value:'by'},{label:'Date',value:'date'},{label:'Status',value:function(r){return '<span class="badge green">'+r.status+'</span>';}}])+'</div></div>';
  },
  insights:function(){
    var a=aggregate();
  ...
  quality:function(){
    ...
    kpi('Missing Data',pct(missRate),'â†“ 1.1pp vs last upload','','â€”')+kpi('Duplicate Records','1.5%','â†“ 0.4pp vs last upload','green','â‰¡')+kpi('Validation Issues','2.8%','â†“ 0.7pp vs last upload','green','!')+'</div>'+
    '<div class="twoCol" style="margin-top:18px"><div class="card"><div class="cardHeader"><h3>Missing Data by Column</h3></div><div class="cardBody">'+qrows(['Driver Phone','Vehicle Model','POD Proof of Delivery','Customer Email'],[64,41,29,12])+'<a class="link">View full data quality report â†’</a></div></div><div class="card"><div class="cardHeader"><h3>Validation Checks</h3></div><div class="cardBody policyList"><div class="policyItem"><b>âœ“ Date format is valid</b><span class="badge green">Passed</span></div><div class="policyItem"><b>âœ“ Trip distance greater than 0</b><span class="badge green">Passed</span></div><div class="policyItem"><b>âœ“ Revenue greater than or equal 0</b><span class="badge green">Passed</span></div><div class="policyItem"><b>â—‹ Driver ID exists in master</b><span class="badge green">Passed</span></div><div class="policyItem"><b>â—‹ Required fields not empty</b><span class="badge orange">Warning</span></div></div></div></div>';
  },
  governance:function(){
    ...
    '<div class="govGrid">'+kpi('AI Usage Register','12','Active use cases','','â–¦')+kpi('Overall AI Risk Level',done>=4?'Low':'Medium','Review risks â†’','orange','!')+kpi('High Risk Use Cases','2','Require attention','purple','âš‘')+kpi('Policy Compliance',Math.round(done/state.checks.length*100)+'%','On track','green','âœ“')+'</div>'+
    '<div class="twoCol" style="margin-top:18px"><div class="card"><div class="cardHeader"><h3>AI Governance Checklist</h3></div><div class="cardBody">'+state.checks.map(function(c,i){return '<label class="checkItem"><input type="checkbox" data-check="'+i+'" '+(c.done?'checked':'')+'><span>'+c.text+'</span><span style="margin-left:auto" class="badge '+(c.done?'green':'orange')+'">'+(c.done?'Done':(i===3?'In progress':'Not started'))+'</span></label>';}).join('')+'</div></div><div class="card"><div class="cardHeader"><h3>Policy Templates</h3></div><div class="cardBody policyList"><div class="policyItem"><b>AI Use Policy</b><span class="badge green">Published</span></div><div class="policyItem"><b>Data Privacy Policy</b><span class="badge green">Published</span></div><div class="policyItem"><b>Model Risk Management Policy</b><span class="badge purple">Draft</span></div><div class="policyItem"><b>AI Ethics Principles</b><span class="badge green">Published</span></div><a class="link">View all policies â†’</a></div></div></div><div style="margin-top:18px">'+stepDoc+'</div>';
  },
  customers:function(){
    return header('Customers','Manage SME accounts and pilot organisations','<button class="primaryBtn" id="addCustomer">Add customer</button><input class="search" id="customerSearch" placeholder="Search customers...">')+
    '<div class="threeCol" id="customerList">'+state.customers.map(customerCard).join('')+'</div><div class="card" style="margin-top:18px"><div class="cardHeader"><h3>Customer onboarding pipeline</h3></div><div class="cardBody">'+barChart([['Demo',7],['Pilot',3],['Paid',1],['Renewal',1]])+'</div></div>';
  },
  settings:function(){
  ...
  billing:function(){
    return header('Billing','Manage subscription, invoices and usage','<button class="primaryBtn" id="invoiceBtn">Download invoice</button>')+
    '<div class="threeCol"><div class="plan"><h3>Starter</h3><p>For early SME pilots</p><div class="price">â‚¬99/mo</div><ul><li>3 uploads/month</li><li>Basic dashboards</li><li>Monthly PDF report</li></ul><button class="plainBtn full" data-plan="Starter">Select</button></div><div class="plan featured"><span class="badge">Current plan</span><h3>Growth</h3><p>For growing transport operators</p><div class="price">â‚¬249/mo</div><ul><li>Unlimited CSV uploads</li><li>AI insights</li><li>Governance checklist</li><li>Report exports</li></ul><button class="primaryBtn full" data-plan="Growth">Current</button></div><div class="plan"><h3>Scale</h3><p>For multi-site fleet companies</p><div class="price">â‚¬699/mo</div><ul><li>Advanced integrations</li><li>Custom governance templates</li><li>Priority support</li></ul><button class="plainBtn full" data-plan="Scale">Select</button></div></div><div class="card" style="margin-top:18px"><div class="cardHeader"><h3>Usage this month</h3></div><div class="cardBody policyList"><div class="policyItem"><b>File uploads</b><span>4 / Unlimited</span></div><div class="policyItem"><b>AI questions</b><span>128 / 500</span></div><div class="policyItem"><b>Reports generated</b><span>7</span></div></div></div>';
  },
  support:function(){
    return header('Support','Get help with uploads, reports, and governance setup','<button class="softBtn" id="docsBtn">Open docs</button>')+
    '<div class="twoCol"><div class="card pad"><h3>Create support ticket</h3><div class="formGrid" style="margin-top:16px"><div class="field"><label>Subject</label><input id="ticketSubject" placeholder="What do you need help with?"></div><div class="field"><label>Priority</label><select id="ticketPriority"><option>Low</option><option selected>Medium</option><option>High</option></select></div><div class="field full"><label>Message</label><textarea id="ticketMessage" rows="6" placeholder="Explain the issue..."></textarea></div><button class="primaryBtn" id="submitTicket">Submit ticket</button></div></div><div class="card"><div class="cardHeader"><h3>Open tickets</h3></div><div class="cardBody policyList">'+state.tickets.map(function(t){return '<div class="policyItem"><div><b>'+escapeHtml(t.subject)+'</b><p>'+escapeHtml(t.priority)+' priority</p></div><span class="badge '+(t.status==='Open'?'orange':'purple')+'">'+escapeHtml(t.status)+'</span></div>';}).join('')+'</div></div></div><div class="card" style="margin-top:18px"><div class="cardHeader"><h3>Quick help</h3></div><div class="cardBody threeCol"><div><h3>Upload guide</h3><p>Use CSV files with date, driver, route, trips, revenue and complaints columns.</p></div><div><h3>Report guide</h3><p>Generate monthly PDF-style reports and export rows as CSV.</p></div><div><h3>Governance guide</h3><p>Keep AI use cases, policies and checklist evidence together.</p></div></div></div>';
  }
};
...
function bindBilling(){ $('#invoiceBtn').onclick=function(){ download('InsightGuard AI Invoice\nPlan: Growth\nAmount: â‚¬249\nStatus: Paid','InsightGuard_Invoice.txt','text/plain'); }; $all('[data-plan]').forEach(function(b){ b.onclick=function(){ toast(b.getAttribute('data-plan')+' plan selected'); }; }); }
function bindSupport(){ $('#docsBtn').onclick=function(){ toast('Docs opened in demo mode'); }; $('#submitTicket').onclick=function(){ var subj=$('#ticketSubject').value.trim(); if(!subj){ toast('Enter a ticket subject'); return; } state.tickets.unshift({subject:subj,status:'Open',priority:$('#ticketPriority').value}); save(); toast('Support ticket created'); render(); }; }
